...
const EMAIL_PRUEBA = "montenegro.rsm@gmail.com"
...
export const recordatorioAntropometrias = async (req, res) => {
  try {
    ...
    if (pendientes.length === 0) {
      return res.json({ message: "Ningún alumno con antropometría pendiente", total: 0 })
    }

    const filas = pendientes.map(a => {
      const ultimaFecha = a.ultima_antro
        ? new Date(a.ultima_antro + "T00:00:00").toLocaleDateString("es-AR")
        : "Sin registro"
      return `
        <tr>
          <td style="padding:10px 14px; border-bottom:1px solid #e5e7eb; font-size:14px;">${a.nombre}</td>
          <td style="padding:10px 14px; border-bottom:1px solid #e5e7eb; font-size:14px; color:#dc2626; font-weight:600;">${ultimaFecha}</td>
        </tr>`
    }).join("")

    const sendSmtpEmail = new brevo.SendSmtpEmail()
    sendSmtpEmail.subject = `📏 ${pendientes.length} alumno${pendientes.length > 1 ? "s" : ""} sin antropometría reciente`
    sendSmtpEmail.sender = {
      name: "RM ENTRENADOR",
      email: process.env.BREVO_SENDER_EMAIL
    }
                ...
                <tr>
                  <td style="background-color:#22b567; padding:16px; text-align:center;">
                    <h1 style="margin:0; font-size:20px; font-weight:600; color:#ffffff;">
                      Recordatorio de Antropometrías
                    </h1>
                  </td>
                </tr>
                    ...
                    ">
                      <thead>
                        <tr style="background-color:#f6fffa;">
                          ...
                          <th style="padding:10px 14px; text-align:left; font-size:13px; color:#6b7280; font-weight:600; border-bottom:1px solid #e5e7eb;">Última antropometría</th>
                        </tr>
                      </thead>
                      <tbody>
                        ${filas}
    ...
    await apiInstance.sendTransacEmail(sendSmtpEmail)
    console.log(`📧 Recordatorio de antropometrías enviado → montenegro.rsm@gmail.com`)

    return res.json({
      message: "Recordatorio enviado",
      total: pendientes.length,
      alumnos: pendientes.map(a => ({ nombre: a.nombre, ultima_antro: a.ultima_antro ?? "Sin registro" }))
    })
  } catch (err) {
...
export const recordatorioCumpleanos = async (req, res) => {
  try {
    ...
    await apiInstance.sendTransacEmail(sendSmtpEmail)
    console.log(`📧 Recordatorio de cumpleaños enviado → montenegro.rsm@gmail.com`)
